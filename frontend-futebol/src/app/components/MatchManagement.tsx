...
import { Save, AlertCircle, CheckCircle, Calendar, RefreshCcw } from 'lucide-react';
...
export const MatchManagement = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // setFormData({...formData, matchId: '', homeTeam: '', awayTeam: ''}); 
    } catch (error: any) {
  ...
  // Helpers para mostrar o NOME do time selecionado, não o ID
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {status.msg && (
              <div className={`p-3 rounded text-sm border ${
                status.type === 'error' ? 'bg-red-950/50 border-red-800 text-red-400' : 
                'bg-emerald-950/50 border-emerald-800 text-emerald-400'
              }`}>
                {status.type === 'error' ? <AlertCircle size={16} className="inline mr-2"/> : <CheckCircle size={16} className="inline mr-2"/>}
                {status.msg}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center justify-center">
                ...
                <div className="space-y-2 p-4 bg-zinc-950/50 rounded border border-zinc-800">
                    ...
                    <Select 
                      ...
                      onValueChange={(v: string) => setFormData({...formData, homeTeam: v})}
                    >
                        <SelectTrigger className="bg-zinc-900 border-zinc-800 text-white">
                          {/* Truque: Se tiver ID, mostra o nome. Se não, mostra "Selecione..." */}
                          <SelectValue placeholder={getTeamName(formData.homeTeam) || "Selecione..."} />
                        </SelectTrigger>
                        <SelectContent className="bg-zinc-900 border-zinc-800 text-white h-48">
                          {teams.map(t => (
                ...
                <div className="space-y-2 p-4 bg-zinc-950/50 rounded border border-zinc-800">
                    ...
                    <Select 
                      ...
                      onValueChange={(v: string) => setFormData({...formData, awayTeam: v})}
                    >
                        <SelectTrigger className="bg-zinc-900 border-zinc-800 text-white">
                          <SelectValue placeholder={getTeamName(formData.awayTeam) || "Selecione..."} />
                        </SelectTrigger>
                        <SelectContent className="bg-zinc-900 border-zinc-800 text-white h-48">
                          {teams.map(t => (
                            <SelectItem key={t.id} value={String(t.id)}>{t.name}</SelectItem>
            ...
            <Button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold h-12">
